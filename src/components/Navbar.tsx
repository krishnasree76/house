    ...
    <nav className="sticky top-0 z-50 glass border-b border-border/50">
      <div className="container mx-auto flex items-center justify-between py-3 px-4">
        ...
        <div className="hidden md:flex flex-1 max-w-md mx-6">
          <div className="relative w-full">
            ...
            <input
              ...
              value={searchQuery}
              onChange={(e) => onSearchChange(e.target.value)}
              className="w-full rounded-full bg-muted/60 border-0 py-2.5 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
            />
          </div>
        </div>
        ...
        <div className="hidden md:flex items-center gap-1">
          <button
            onClick={() => navigate("/profile")}
            className="p-2.5 rounded-full hover:bg-muted/60"
          >
            <User className="h-5 w-5" />
          </button>

          <Link to="/orders" className="p-2.5 rounded-full hover:bg-muted/60">
            <Package className="h-5 w-5" />
          </Link>

          <Link to="/wishlist" className="relative p-2.5 rounded-full hover:bg-muted/60">
            <Heart className="h-5 w-5" />
            {wishlistCount > 0 && (
              <span className="absolute -top-0.5 -right-0.5 h-4 w-4 bg-primary text-[10px] flex items-center justify-center rounded-full text-white">
                {wishlistCount}
              </span>
            )}
          </Link>

          <Link to="/cart" className="relative p-2.5 rounded-full hover:bg-muted/60">
            <ShoppingBag className="h-5 w-5" />
            {cartCount > 0 && (
              <span className="absolute -top-0.5 -right-0.5 h-4 w-4 bg-primary text-[10px] flex items-center justify-center rounded-full text-white">
                {cartCount}
          ...
          <button
            onClick={onLoginOpen}
            className="ml-2 bg-foreground text-background px-5 py-2 rounded-full text-sm"
          >
            Login
          </button>
        </div>
      ...
      <div className="md:hidden px-4 pb-3">
        <div className="relative">
          ...
          <input
            ...
            value={searchQuery}
            onChange={(e) => onSearchChange(e.target.value)}
            className="w-full rounded-full bg-muted/60 py-2.5 pl-10 pr-4 text-sm"
          />
        </div>
      </div>
        ...
        {mobileOpen && (
          ...
          >
            <div className="p-4 space-y-2">
              <button onClick={() => navigate("/profile")} className="flex gap-3 p-3">
                <User /> Profile
              </button>
